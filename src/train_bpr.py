...
def main():
    ...
    if VALIDATION:
        logURL = 'log/latent_{0}_epoch_{1}_neg_{2}_{3}.bpr.log'.format(LATENT_DIM, NUM_OF_EPOCH, NEG_RATIO, datasetFileURL)
    ...
    # write model
    if not VALIDATION:
        torch.save(matrixFactorizationModel.state_dict(), '{0}latent_{1}_epoch_{2}_neg_{3}_{4}.bpr'.format('model/', LATENT_DIM, NUM_OF_EPOCH, NEG_RATIO, datasetURL))
        outputPath = '{0}latent_{1}_epoch_{2}_neg_{3}_{4}.bpr.out'.format('output/', LATENT_DIM, NUM_OF_EPOCH, NEG_RATIO, datasetURL)
